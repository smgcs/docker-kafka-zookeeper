...
# * ADVERTISED_ADDR: the external ip:port for the container, e.g. `docker-machine ip \`docker-machine active\``
# * ZK_CHROOT: the zookeeper chroot that's used by Kafka (without / prefix), e.g. "kafka"
# * LOG_RETENTION_HOURS: the minimum age of a log file in hours to be eligible for deletion (default is 168, for 1 week)
# * LOG_RETENTION_BYTES: configure the size at which segments are pruned from the log, (default is 1073741824, for 1GB)
# * NUM_PARTITIONS: configure the default number of log partitions per topic
# * LOG_DIRS: A comma separated list of directories under which to store log files 

# Set the external host and port
if [ ! -z "$ADVERTISED_ADDR" ]; then
    echo "advertised addr: $ADVERTISED_ADDR"
    # Uncomment advertised.listeners
    sed -r -i 's/^(#)(advertised.listeners)/\2/g' $KAFKA_HOME/config/server.properties

    # Replace your.host.name:9092 with $ADVERTISED_ADDR
    sed -r -i "s/your.host.name:9092/$ADVERTISED_ADDR/g" $KAFKA_HOME/config/server.properties
fi
